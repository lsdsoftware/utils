...
export function connectSocket(options) {
    return rxjs.defer(() => {
        ...
        if (callerSignal?.aborted)
            abortCtl.abort(callerSignal.reason);
        ...
        if (callerSignal && !callerSignal.aborted) {
            rxjs.fromEvent(callerSignal, 'abort', () => callerSignal.reason).pipe(rxjs.takeUntil(conn.close$)).subscribe(reason => abortCtl.abort(reason));
        }
        return rxjs.race(conn.error$.pipe(rxjs.map(err => { throw err; })), rxjs.fromEvent(sock, 'connect').pipe(rxjs.take(1), rxjs.map(() => conn))).pipe(
        ...
        rxjs.share({ resetOnRefCountZero: false }), finalizeWithReason(reason => {
...
function makeConnection(sock) {
    return {
        socket: sock,
        data$: rxjs.fromEvent(sock, 'data', (chunk) => chunk).pipe(rxjs.takeUntil(rxjs.fromEvent(sock, 'end'))),
        ...
        close$: rxjs.from(new Promise(f => sock.once('close', f))),
    };
}
